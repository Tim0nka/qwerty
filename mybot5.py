...
def log_action(user, motion: str, api: str | None, answer: str | None):
    now = datetime.now()
    user_display = f"{user.full_name} (@{user.username})" if user.username else user.full_name
    log_row = [
...
async def handle_callback(callback: CallbackQuery):
    ...
    if action == "weather":
        await get_weather(callback.message, user)
    elif action == "currency":
        await get_currency(callback.message, user)
    elif action == "bitcoin":
        await get_bitcoin(callback.message, user)
    else:
...
#Погода
async def get_weather(message: types.Message, user: types.User):
    city = "Minsk"
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            ...
            if resp.status == 200:
                ...
                log_action(user, "weather", "OpenWeatherMap", answer)
            else:
                await message.answer("Не удалось получить данные о погоде")


#Курс USD в BYN
async def get_currency(message: types.Message, user: types.User):
        ...
        async with session.get(url) as resp:
            ...
            if resp.status == 200:
                ...
                if byn:
                    ...
                    await message.answer(answer)
                    #Добавляем логирование
                    log_action(user, "currency", "ExchangeRate API", answer)
                else:
...
#Курс Биткойна
async def get_bitcoin(message: types.Message, user: types.User):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            ...
            if resp.status == 200 and "bitcoin" in data:
                ...
                await message.answer(answer)
                #Добавляем логирование
                log_action(user, "bitcoin", "CoinGecko", answer)
            else:
